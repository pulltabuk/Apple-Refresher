...
2. Open the SQL editor and run, in order: `supabase-schema.sql`, then
   ...
   then `supabase-schema-update-3.sql`, then
...
- `SITE_URL` must be the public domain. It defaults to
  ...
  and RSS is built from it, and the build writes a 301 from the
  netlify.app address so only one copy of the site is indexable.
...
- Products are organised as family (Category) > product line (a product)
  ...
  optional name and announced date, stored in `generation_details` keyed
  by its date. Generations without a typed name get one automatically,
  e.g. "AirPods Pro (2nd generation)".
